...
<div class="space-y-6">
	{#if collections.length > 1 && collections.length < 5}
		<!-- Side by side layout for buttons -->
		<div class="flex flex-col space-y-4 lg:flex-row lg:space-y-0 lg:space-x-8">
			<!-- Left side - Collection buttons -->
		...
		<!-- Helper text and upload limit below -->
		<div class="flex flex-col space-y-4 lg:flex-row lg:space-y-0 lg:space-x-8">
	...
	{:else}
		<!-- Two column layout for dropdown version (5+ collections) -->
		<div
			class="flex flex-col items-center space-y-4 lg:flex-row lg:items-start lg:space-y-0 lg:space-x-8"
		>
			<!-- Left column - Collection selector (dropdown only) and text content -->
			<div class="w-full space-y-4 lg:w-1/2">
				<!-- Collection Selector (Desktop - Dropdown only for 5+ collections) -->
				...
				<!-- Mobile Collection Selector (Dropdown only for 5+ collections) -->
				{#if collections.length >= 5}
			...
			<div class="flex w-full flex-col space-y-4 {collections.length >= 5 ? 'lg:w-1/2' : ''}">
				<FileUploadButton
					{user}
					{uploadLimit}
					{currentCollectionUuid}
