<script lang="ts">
	import ManualCodeEntry from '$lib/components/ManualCodeEntry.svelte';
	import { setupKeyboardNavigation, shouldUseTVUI } from '$lib/tvUtils';
	import { onMount, onDestroy } from 'svelte';

	export let showModal: boolean;
	export let onClose: () => void;
	export let onSuccess: () => void;

	let modalElement: HTMLElement;
	let keyboardNavCleanup: (() => void) | null = null;
	let isTVDevice = false;

	// Setup TV navigation when modal opens
	$: if (showModal) {
		setupTVNavigation();
	} else {
		cleanupTVNavigation();
	}

	function setupTVNavigation() {
		if (modalElement && isTVDevice && showModal) {
			// Small delay to ensure modal is fully rendered
			setTimeout(() => {
				keyboardNavCleanup = setupKeyboardNavigation(modalElement);
			}, 100);
		}
	}

	function cleanupTVNavigation() {
		if (keyboardNavCleanup) {
			keyboardNavCleanup();
			keyboardNavCleanup = null;
		}
	}

	function handleKeydown(event: KeyboardEvent) {
		if (event.key === 'Escape') {
			onClose();
		}
		// Don't handle other keys here if TV navigation is active
		// Let setupKeyboardNavigation handle arrow keys
	}

	onMount(async () => {
		isTVDevice = await shouldUseTVUI();
	});

	onDestroy(() => {
		cleanupTVNavigation();
	});
</script>

{#if showModal}
	<div
		class="bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black"
		on:keydown={handleKeydown}
		role="dialog"
		aria-modal="true"
		tabindex="-1"
	>
		<div
			bind:this={modalElement}
			class="relative w-full max-w-md rounded-lg bg-white p-6 shadow-xl"
		>
			<button
				class="btn btn-circle btn-ghost btn-sm absolute top-2 right-2 {isTVDevice
					? 'focus:ring-4 focus:ring-blue-500 focus:ring-offset-2'
					: ''}"
				on:click={onClose}
			>
				✕
			</button>
			<ManualCodeEntry {onSuccess} {isTVDevice} />
		</div>
	</div>
{/if}
